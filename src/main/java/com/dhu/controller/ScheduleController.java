...
public class ScheduleController {
    ...
    public Result querySchedule(@RequestParam int current, @RequestParam int size, @RequestParam String search, @RequestParam("finished") String isFinished) {
        return Result.nullFilterData("schedules", scheduleService.querySchedule(current, size, UserHolder.getUser().getId(), search, Boolean.parseBoolean(isFinished)));
    }

    //查询计划中论文列表
    @GetMapping("/papers")
    public Result queryPaperBySchedule(@RequestParam int current, @RequestParam int size, @RequestParam("schedule_id") Integer scheduleId, @RequestParam("finished") Boolean isFinished) {
        return Result.nullFilterData("papers", scheduleService.queryPaperBySchedule(current, size, scheduleId, isFinished));
    }
    ...
    public Result addPaperRelation(SchedulePaperRelation schedulePaperRelation){
    ...
    public Result deletePaperRelation(Integer scheduleId, Integer paperId){
    ...
    public Result deletePaperRelations(Integer scheduleId, List<Integer> paperIds){
        return Result.verifyDelete(schedulePaperRelationService.deletePaperRelations(scheduleId, paperIds));
    }

    @PostMapping("/rel_update")
    //设置论文状态
    public Result setPaperStatus(SchedulePaperRelation schedulePaperRelation){
