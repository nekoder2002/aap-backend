...
public class PaperController {
    ...
    Result queryList(@RequestParam int current,@RequestParam int size, @RequestParam("kb_id") Integer kbId) {
        ...
        return Result.nullFilterData("kbs", paperService.queryPapers(current, size, kbId));
    }

    //上传论文
    @PutMapping("/insert")
    Result insert(@RequestBody PapersAddDTO papersAddDTO, MultipartFile[] files){
        return Result.verifySave(paperService.uploadPaper(papersAddDTO,files));
    }

    //删除论文
    @DeleteMapping("/delete")
    Result deletePaper(@RequestParam Integer paperId){
    ...
    Result deletePapersByKb(@RequestBody List<Integer> paperIds, @RequestParam Integer kbId){
