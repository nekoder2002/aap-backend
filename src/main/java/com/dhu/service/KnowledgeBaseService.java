...
public interface KnowledgeBaseService {
    //查询单个知识库信息
    KbDTO querySingle(Integer kbId, Integer userId);

    //查询前十个自己的知识库
    List<KbDTO> queryKbLimit(Integer userId, int limit);

    //获取团队的知识库列表
    IPage<KbDTO> queryTeamKnowledgeBases(int current, int size, Integer teamId, Integer userId, String search);

    //获取个人的知识库列表
    IPage<KbDTO> queryUserKnowledgeBases(int current, int size, Integer userId, String search);
