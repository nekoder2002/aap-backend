package com.dhu.constants;

public class InterfaceUrlConstants {
    public final static String GET_KNOWLEDGE_BASE_LIST = "/knowledge_base/list_knowledge_bases";
    public final static String ADD_KNOWLEDGE_BASE = "/knowledge_base/create_knowledge_base";
    public final static String DEL_KNOWLEDGE_BASE = "/knowledge_base/delete_knowledge_base";
    public final static String DEL_FILE = "/knowledge_base/delete_docs";
    public final static String UPLOAD_FILE = "/knowledge_base/upload_docs";
    public final static String PAPER_CHAT = "/chat/single_chat";
    public final static String KB_CHAT = "/chat/knowledge_base_chat";
}
