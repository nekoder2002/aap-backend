package com.dhu.constants;

public class BaseConstants {
    public static final String PROJECT_NAME = "论文辅助阅读工具平台";
    public static final int QUICK_SEARCH_NUM = 10;
    public static final int STUDY_COUNT_NUM = 20;
    public static final int CHAT_NUM = 100;
    public static final String PAPER_TYPE = ".pdf";
    public static final int MAIL_CAPTCHA_LENGTH = 6;
    public static final int TEAM_CODE_LENGTH = 32;
    public static final int LOG_SHOW_LIMIT = 100;
}